...
eps = 1e-6
#the code is taken from https://github.com/sneddy/pneumothorax-segmentation/blob/master/unet_pipeline/Losses.py
def dice_loss_4img(in_tensor):
    loss = 0
    dice_loss = DiceLoss()
    cnt = 0
    ...
    for i in range(4):
        for j in range(i + 1, 4):
            cnt += 1
            loss += dice_loss(in_tensor[:, i, :, :], in_tensor[:, j, :, :])
    return loss/cnt
